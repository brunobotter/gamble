...
public interface GambleRepository extends JpaRepository<Gamble, Long> {
    ...
    @Query("SELECT new br.com.bruno_botter.gamble.dto.ConsultaResponse(c.saldo, c.name, c.username, " +
            ...
            "FROM Client c JOIN Gamble g ON c.clientId = g.client.clientId WHERE c.clientId = :clientId")
    List<ConsultaResponse> findClientAndGambleByClientId(@Param("clientId") Long clientId);
}
